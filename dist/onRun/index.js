...
const onRun = async (config) => {
    const { $: { perfStart, isInsideDockerContainer, hostname }, opts: { afterSetupSleep, dev: { debug }, }, runners, } = config;
    await dockerComposeUp_1.default(runners.map(runner => runner.runnerConfig.service));
